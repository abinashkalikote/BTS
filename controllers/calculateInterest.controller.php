...
if(isset($_SERVER['REQUEST_METHOD']) == 'POST' && isset($_POST['calculateinterest'])){
    ...
    // calculationupto is defined if the calculationupto is NULL then set the last transaction date as calculationupto
    if($calculationupto != NULL){
        $calculationupto = $calculationupto;
    }else{
        ...
        if($result->num_rows > 0){
            ...
            $calculationupto  = $row['lasttransactiondate'];
        }
    }


    $currentDate = $_SESSION['logindate'];
    define('CURRENTDATE', $currentDate);

    if($calculationupto > CURRENTDATE){
        $_SESSION['interestnotcalculated'] = "Interest Can't be calculated on future date !";
        ...
        if($interestrate != 0){
            ...
            $dr = 0;
            $cr = 0;
            $q = "SELECT amount, drcr, rec_Status FROM fin_transactions WHERE account_No='$accountno' AND rec_Status='A' AND transaction_Date <= '$calculationupto'";
            $result = $conn->query($q);
            if($result->num_rows > 0){
                echo $interestrate;
